window.Ranch = {
    ...
    loadAuthState() {
        try {
            ...
            if (auth) {
                ...
                // NO AUTH? NO PROBLEM. Load demo mode so visitors see everything immediately.
                this.state.authenticated = false;
                this.state.user = {
                    email: 'demo@ranch.local',
                    ranchName: 'Sunset Ranch',
                    tier: 'FREE'
                };
                this.state.tier = 'FREE';
            }
            // ALWAYS show the app — never block with auth screen
            this.showApp();
        } catch (e) {
            console.warn('[RANCH] Auth load failed:', e);
            this.showApp(); // fallback: still show app
        }
    },
    saveAuthState(user) {
        this.state.authenticated = true;
    ...
    clearAuthState() {
        ...
        // Instead of showing auth screen, reload demo mode
        this.loadAuthState();
        this.showToast('Logged out. Demo mode active.', 'info');
    },
    showAuth() {
        // DEPRECATED: Auth screen no longer blocks the app.
        // Users can log in from Settings if they want to save data.
        this.showApp();
    },
    showApp() {
